...
def add_points(image,masks, n_instances, class_ids, anteriores=None):
    ...
    for i in range(n_instances):
        ...
        if class_ids[i] == 2: 
            ...
            media_instance_interrow.append(np.mean(np.array(medias_linha)))
    ...
    r_d1 = has_point(media_instance_row, r1 , distance_between_rows)
    print("r_d1", r_d1)
    r_d2 = has_point(media_instance_row, r2, distance_between_rows)
    print("r_d2", r_d2)
    # r_d3 = has_point(media_instance_row, r3, distance_between_rows)
    # print("r_d3", r_d3)
    ir_d1 = has_point(media_instance_interrow, ir1 , distance_between_rows)
    print("ir_d1", ir_d1)
    # ir_d2 = has_point(media_instance_interrow, ir2, distance_between_rows)
    # print("ir_d2", ir_d2)
    ir_d3 = has_point(media_instance_interrow, ir3, distance_between_rows)
    print("ir_d3", ir_d3)
    ...
    points.append((ir1 + delta, int(height_points*height)+30))
    # points.append((ir2 + delta, int(height_points*height)+30))
    points.append((ir3 + delta, int(height_points*height)+30))
   

    for i, point in enumerate(points):
        image = cv2.circle(image, point, radius=20, color=(0, 0, 255), thickness=-1)
